...
public partial class UIModBuffEditorPanel : UIModCommonEditorFramePanel
{
    ...
    protected override void OnInitEditor()
    {
        ...
        BuffTypeDrawer.SetOptions(ModMgr.I.BuffDataBuffTypes.Select(item => $"{item.TypeID} {item.TypeName}").ToList());
        BuffTypeDrawer.ValueChange = index =>
        {
            var buffType = ModMgr.I.BuffDataBuffTypes[index];
            SelectedItem.BuffType = buffType.TypeID;
        };

        BuffTriggerTypeDrawer.Title = "触发类型";
        BuffTriggerTypeDrawer.SetOptions(ModMgr.I.BuffDataTriggerTypes.Select(item => $"{item.ID} {item.Desc}").ToList());
        BuffTriggerTypeDrawer.ValueChange = index =>
        {
            var buffTrigger = ModMgr.I.BuffDataTriggerTypes[index];
            SelectedItem.Trigger = buffTrigger.ID;
        };

        BuffRemoveTriggerTypeDrawer.Title = "移除类型";
        BuffRemoveTriggerTypeDrawer.SetOptions(ModMgr.I.BuffDataRemoveTriggerTypes.Select(item => $"{item.ID} {item.Desc}").ToList());
        BuffRemoveTriggerTypeDrawer.ValueChange = index =>
        {
            var buffRemoveTrigger = ModMgr.I.BuffDataRemoveTriggerTypes[index];
            SelectedItem.RemoverTrigger = buffRemoveTrigger.ID;
        };
        
        BuffOverlayTypeDrawer.Title = "叠加类型";
        BuffOverlayTypeDrawer.SetOptions(ModMgr.I.BuffDataOverlayTypes.Select(item => $"{item.ID} {item.Desc}").ToList());
        BuffOverlayTypeDrawer.ValueChange = index =>
        {
            var buffOverlayType = ModMgr.I.BuffDataOverlayTypes[index];
    ...
    protected override void OnEditorRefresh(IModData data)
    {
        ...
        BuffTypeDrawer.Select(ModMgr.I.BuffDataBuffTypes
            .TryFind(item => item.TypeID == curData.BuffType));
        BuffTriggerTypeDrawer.Select(ModMgr.I.BuffDataTriggerTypes
            .TryFind(item => item.ID == curData.Trigger));
        BuffRemoveTriggerTypeDrawer.Select(ModMgr.I.BuffDataRemoveTriggerTypes
            .TryFind(item => item.ID == curData.RemoverTrigger));
        BuffOverlayTypeDrawer.Select(ModMgr.I.BuffDataOverlayTypes
