            ...
            switch (drawerId)
            {
                case "ComparisonOperatorTypeDrawer":
                {
                    ...
                    void OnDrawerRefresh()
                    {
                        ...
                        var typeList = ModMgr.I.ComparisonOperatorTypes
                            .Select(type => $"{type.ID} {type.Desc}").ToList();
                        targetDrawer.SetOptions(typeList);
                        var typeIndex = ModMgr.I.ComparisonOperatorTypes.FindIndex(type => type.ID == typeValue);
                        targetDrawer.Select(typeIndex);
                        targetDrawer.ValueChange = index =>
                        {
                            var typeId = ModMgr.I.ComparisonOperatorTypes[index].ID;
        ...
        foreach (var drawerId in seidProperty.SpecialDrawer)
        {
            switch (drawerId)
            {
                case "BuffDrawer":
                {
                    ...
                    void OnDrawerRefresh()
                    {
                        var buffData = ModMgr.I.CurProject.FindBuff(sInt.Value);
                        if (buffData != null)
                        {
                ...
                case "SeidDrawer":
                {
                    ...
                    void OnDrawerRefresh()
                    {
                        if (ModMgr.I.BuffSeidMetas.TryGetValue(sInt.Value,out var seidMeta))
                        {
                ...
                case "BuffTypeDrawer":
                {
                    ...
                    void OnDrawerRefresh()
                    {
                        ...
                        var typeList = ModMgr.I.BuffDataBuffTypes
                            .Select(type => $"{type.TypeID} {type.TypeName}").ToList();
                        seidDrawer.SetOptions(typeList);
                        var typeIndex = ModMgr.I.BuffDataBuffTypes.FindIndex(type => type.TypeID == typeValue);
                        seidDrawer.Select(typeIndex);
                        seidDrawer.ValueChange = index =>
                        {
                            var typeId = ModMgr.I.BuffDataBuffTypes[index].TypeID;
                ...
                case "BuffRemoveTriggerTypeDrawer":
                {
                    ...
                    {
                        var typeValue = sInt.Value;
                        var typeList = ModMgr.I.BuffDataRemoveTriggerTypes
                            .Select(type => $"{type.ID} {type.Desc}").ToList();
                        seidDrawer.SetOptions(typeList);
                        var typeIndex = ModMgr.I.BuffDataRemoveTriggerTypes.FindIndex(type => type.ID == typeValue);
                        seidDrawer.Select(typeIndex);
                        seidDrawer.ValueChange = index =>
                        {
                            var typeId = ModMgr.I.BuffDataRemoveTriggerTypes[index].ID;
                            inputNumberDrawer.Content = typeId.ToString();
                        };
                    }
                ...
                case "AttackTypeDrawer":
                {
                    ...
                    {
                        var typeValue = sInt.Value;
                        var typeList = ModMgr.I.AttackTypes
                            .Select(type => $"{type.ID} {type.Desc}").ToList();
                        seidDrawer.SetOptions(typeList);
                        var typeIndex = ModMgr.I.AttackTypes.FindIndex(type => type.ID == typeValue);
                        seidDrawer.Select(typeIndex);
                        seidDrawer.ValueChange = index =>
                        {
                            var typeId = ModMgr.I.AttackTypes[index].ID;
                            inputNumberDrawer.Content = typeId.ToString();
                        };
                    }
                ...
                case "ElementTypeDrawer":
                {
                    ...
                    {
                        var typeValue = sInt.Value;
                        var typeList = ModMgr.I.ElementTypes
                            .Select(type => $"{type.ID} {type.Desc}").ToList();
                        attackTypeDrawer.SetOptions(typeList);
                        var typeIndex = ModMgr.I.ElementTypes.FindIndex(type => type.ID == typeValue);
                        attackTypeDrawer.Select(typeIndex);
                        attackTypeDrawer.ValueChange = index =>
                        {
                            var typeId = ModMgr.I.ElementTypes[index].ID;
                            inputNumberDrawer.Content = typeId.ToString();
                        };
                    }
                ...
                case "TargetTypeDrawer":
                {
                    ...
                    {
                        var typeValue = sInt.Value;
                        var typeList = ModMgr.I.TargetTypes
                            .Select(type => $"{type.ID} {type.Desc}").ToList();
                        targetDrawer.SetOptions(typeList);
                        var typeIndex = ModMgr.I.TargetTypes.FindIndex(type => type.ID == typeValue);
                        targetDrawer.Select(typeIndex);
                        targetDrawer.ValueChange = index =>
                        {
                            var typeId = ModMgr.I.TargetTypes[index].ID;
                            inputNumberDrawer.Content = typeId.ToString();
                        };
                    }
                ...
                case "BuffArrayDrawer":
                {
                    ...
                    void OnBuffDrawerRefresh()
                    {
                        if (sIntArray.Value.Count > 0)
                        {
                            ...
                            {
                                var buffData = ModMgr.I.CurProject.FindBuff(buffId);
                                if (buffData != null)
                                {
                ...
                case "SeidArrayDrawer":
                {
                    ...
                    void OnSeidDrawerRefresh()
                    {
                        if (sIntArray.Value.Count > 0)
                        {
                            ...
                            {
                                if (ModMgr.I.BuffSeidMetas.TryGetValue(seidId,out var seidMeta))
                                {
