...
public partial class UIModAffixEditorPanel : UIModCommonEditorFramePanel
{
    ...
    protected override void OnInitEditor()
    {
        ...
        ProjectTypeDrawer.SetOptions(ModMgr.I.AffixDataProjectTypes.Select(item => $"{item.TypeNum} {item.TypeName}").ToList());
        ProjectTypeDrawer.ValueChange = index =>
        {
            var projectType = ModMgr.I.AffixDataProjectTypes[index];
            SelectedItem.SetProjectType(projectType);
        };
        
        TypeDrawer.Title = "词缀类型";
        TypeDrawer.SetOptions(ModMgr.I.AffixDataAffixTypes.Select(item => $"{item.ID} {item.Name}").ToList());
        TypeDrawer.ValueChange = index =>
        {
            var projectType = ModMgr.I.AffixDataAffixTypes[index];
    ...
    protected override void OnEditorRefresh(IModData data)
    {
        ...
        ProjectTypeDrawer.Select(ModMgr.I.AffixDataProjectTypes
            .TryFind(item => item.TypeNum == curData.ProjectTypeNum));
        TypeDrawer.Select(ModMgr.I.AffixDataAffixTypes
