...
public class UIModItemEditorPanel : UIModCommonEditorFramePanel
{
    ...
    protected override void OnInitEditor()
    {
        ...
        GuideTypeDrawer.SetOptions(ModMgr.Instance.ItemDataGuideTypes.Select(item => $"{item.ID} {item.Desc}").ToList());
        GuideTypeDrawer.ValueChange = index =>
        {
            var guideType = ModMgr.Instance.ItemDataGuideTypes[index];
    ...
    protected override void OnEditorRefresh(IModData data)
    {
        ...
        GuideTypeDrawer.Select(ModMgr.Instance.ItemDataGuideTypes
